...
public class ServerHandler implements HttpHandler {
    ...
    private Response doPut(HttpExchange httpExchange) {
        try {
            Message message = MessageHelper.getClientMessageForEdit(httpExchange.getRequestBody());
            if (messageStorage.updateMessage(message)){return Response.ok();}
            else return Response.badRequest("No such message found");
        } catch (ParseException e) {
            logger.error("Could not parse message.", e);
            return new Response(Constants.RESPONSE_CODE_BAD_REQUEST, "Incorrect request body");
        }
    ...
    private Response doDelete(HttpExchange httpExchange) {
        ...
        if (messageStorage.removeMessage(id)){return Response.ok();}
        else return Response.badRequest("No such message found");
    }
